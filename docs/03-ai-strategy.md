...
MVP は **Haiku 4.5 と Sonnet 4.6 のみ** に絞る。

| 用途 | モデル |
|---|---|
| 問題生成 (全タイプ) | Sonnet 4.6 |
| 採点 (mcq/cloze 以外) | Sonnet 4.6 (重要度高)、short は Haiku 4.5 |
| NL パース | Haiku 4.5 |
| 誤概念抽出 | Sonnet 4.6 |
| 反論時の再採点 | Sonnet 4.6 (同じモデルで再試行) |

Opus 4.7 は **Phase 2 以降**、Sonnet の品質に不足を感じた場面に限って導入する。
...
| 用途 | モデル | 理由 |
|---|---|---|
| 問題生成 (MVP 全般) | **Claude Sonnet 4.6** | コスパ良い、品質十分 |
| 問題生成 (Phase 2+: senior 以上 / design / 高難度 written) | **Claude Opus 4.7** | 推論の質が段違い。MVP では使わない |
...
2. 押下 → 再採点 (同モデルで別プロンプト、Haiku 判定なら Sonnet で再評価)
...
個人用途なので API コスト $2.5/月 + インフラ (Turso 無料枠内、Vercel Hobby) で **実質 $2-3/月** 程度で回る想定。自己投資として全く許容範囲。
...
- API コストが暴走しないよう、**月間予算アラート**を Anthropic 側で設定 (例: $20 超えたら通知)

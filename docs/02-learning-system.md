...
### 2.1.1. 初期ドメイン (8 ドメイン × 平均 30 concept = 240 concept)

```
domains:
  - os            # OS、プロセス、メモリ、スケジューリング
  - network       # TCP/IP、TLS、DNS、HTTP、BGP
  - security      # 認証/認可、暗号、OWASP、Web 攻撃
  - db            # RDB、NoSQL、トランザクション、インデックス
  - distributed   # CAP、consensus、replication、sharding
  - low_level     # アセンブリ、キャッシュ、メモリモデル
  - languages     # 型システム、GC、メモリ管理
  - practical     # Git 内部、Docker、Linux コマンド、観測性
```

### 2.1.2. Concept の定義 (YAML シード例)
